> **Superseded in revision 0.5:** the user chose protected batteries with intact factory PCMs. No central ABLIC stage is populated. This document retains the earlier sourcing history; see [current implementation](pack-protection-implementation.md).

# 1S protector sourcing — 2026-09-12

Historical revision 0.4: S-821AAAC-H8T7S was selected by user direction and implemented as U27; revision 0.5 removes that circuit. No harness change is required. See [implementation and validation](pack-protection-implementation.md). Historical screening below predates the user's prototype selection.
