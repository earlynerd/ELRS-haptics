...
SW3 connects BQ25186 TS/MR to GND, in parallel with R62 (10k). The charger has no separate thermistor; the seven cell temperatures on pods 1-7 are read by their local M2003 ADCs; pod 0 has no required cell sensor. SW1/SW2 are removed. SW3 is the only fitted switch: Panasonic EVPAWBD4A, 1.6 N, 0.6 mm high, top-actuated on the outward B.Cu face (2026-09-17). See [PCB-faced construction](pcb-sandwich.md). J3 is a bare underside EN/GND/BOOT pad bank; provide external access appropriate to the stacked enclosure. The charger can wake SYS from ship mode with the button or valid input power. Its long-press action is programmable, and short-press events reach CHG_nINT. These functions require firmware configuration and button behavior validation.
...
J3 pin 1 is EN, pin 2 GND and pin 3 BOOT/GPIO9. Ground BOOT, briefly ground EN, release EN while keeping BOOT low through reset, then release BOOT and flash over USB. The reset RC and boot pull-ups remain. These pads are for firmware recovery when USB cannot enter download mode automatically; normal USB flashing/JTAG does not need switches. The three 1 mm pads sit at (45.5, 55.0) mm on B.Cu. Keep them accessible through a service opening without lifting the battery. No recovery connector is fitted.

The older enclosure sketches are provisional. SW3 now presses from the outward PCB face; LED visibility and service access must follow the current placement and PCB-faced construction.
...
With USB absent, **BQ25186 ship current is 3.2 uA typical, 5 uA maximum under its specified conditions**, with downstream SYS disconnected. Seven factory PCMs at the YDL example's 1 uA typical each give approximately **10.2 uA plus residual leakage** for the complete bracelet. All seven PCMs remain attached to their cells in ship mode; confirm the actual shipped PCM specification. The central U27 no longer exists. Charger shutdown is 15 nA typical but loses button wake. Neither figure describes charging; charger input IQ at 5 V, 4.5 V SYS and zero charge current is 0.75 mA typical.
