...
for ref,f in fps.items():
 x,y=xy(f.GetPosition());f.BuildCourtyardCaches();bb=f.GetCourtyard(p.F_CrtYd).BBox();x0,y0,x1,y1=[mm(v) for v in [bb.GetX(),bb.GetY(),bb.GetRight(),bb.GetBottom()]]
 if ref=='U1':x0,y0,x1,y1=38.65,40.1,52.35,51.6
 if ref=='M1':x0,y0,x1,y1=x-1.1,y-1.5,x+1.1,y+1.5
 c=col.get(ref,'#ccc')
 s.append(f'<rect x="{x0}" y="{y0}" width="{x1-x0}" height="{y1-y0}" fill="{c}" fill-opacity=".09" stroke="{c}" stroke-width=".045"/>')
 for pad in f.Pads():
 ...
 s.append(f'<text x="{x}" y="{y-.15}" fill="{c}" text-anchor="middle" font-size="{size}" stroke="#111923" stroke-width=".09" paint-order="stroke">{ref}</text>')
