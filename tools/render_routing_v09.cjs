...
const out='hardware/verification/routing-v09/';
(async()=>{
  ...
  const head=`<svg width="1780" height="90"><rect width="1780" height="90" fill="#10161f"/><text x="20" y="30" font-size="22" font-family="sans-serif" fill="white">Satellite pod 6 - revision 0.9</text>${labels.map((s,i)=>`<text x="${20+i*440}" y="66" font-size="17" font-family="sans-serif" fill="#cbd5e1">${s}</text>`).join('')}</svg>`;
