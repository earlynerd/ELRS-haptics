"""Check and package the v0.11 prototype without changing earlier releases."""
...
assert json.loads((OUT/'revision-checks.json').read_text())['passed']
expected={'rigid-strip.stl':9,'tpu-end-flexures.stl':2,'satellite-lid.stl':1,
          ...
          'main-transition-rigid.stl':2,'main-transition-tpu.stl':2}
...
previous=ROOT/'mechanical/haptic-myo-flat-v0.10.zip'
...
report={'passed':True,'expected_mesh_components':expected,'three_mf_packages':packages,
        ...
        'previous_zip_sha256':old_hash,'cad_generator_sha256':digest(ROOT/'tools/build_myo_flat_v011.py'),
...
files += [ROOT/'tools'/name for name in ['build_myo_flat_v011.py','render_myo_flat_v011.py',
          'build_flat_band.py','build_myo_band.py','kicad_edit.py']]
files += [ROOT/name for name in cad['source_hashes']]
target=ROOT/'mechanical/haptic-myo-flat-v0.11.zip'
