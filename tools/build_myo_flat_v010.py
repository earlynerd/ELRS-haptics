...
def screw_positions(main):
    return [(x,y) for y in [-32.5,32.5] for x in [3.3,20.7]] if main else [(10,-19.5),(10,19.5)]
...
def split_main(main):
    """Stepped clamshell: whole wrist floor and whole outer face.

    The lower part owns the right-side full-height wall; the upper owns the
    left-side wall. They print and close at the same Z datum, so ribbon ends
    meet without a height change. Small lower standoffs retain four screws.
    """
    interior=plate(12,0,24-2*WALL,64-2*WALL,15,-.1,r=.3)
    perimeter=main-interior
    # Keep the complete wrist-facing floor, including its perimeter, in lower.
    upper_wall=perimeter&box(-1,-40,FLOOR,WALL_HANDOFF_X+1,80,15)
    lower=main-upper_wall
    upper=upper_wall+Pos(0,0,DEPTH)*lid(True)
    # Two recessed left-side screws require posts belonging to the lower half.
    for y in [-32.5,32.5]:
        x=3.3
        foot=plate(x,y,4.4,4.4,FLOOR,r=.4)
        post=cyl(x,y,0,1.8,DEPTH)
        lower+=foot+post
        upper-=plate(x,y,4.8,4.8,FLOOR+.2,-.1,r=.6)
        upper-=cyl(x,y,-.1,2.0,DEPTH+.1)
    for x,y in screw_positions(True):
        lower-=cyl(x,y,4.5,.8,5.1)
...
def build(wrist,wall):
    ...
    main,mb=shell(True);sat,sb=shell(False);lower,upper=split_main(main)
    origins=[0.]
    ...
    for sign in [-1,1]:
        ...
        for i in range(9):
            ...
            strip=plate(x+w/2,sign*(h/2-1.2),w,RAIL_WIDTH,DEPTH,r=.7)
            # Straight butt ends exactly at the complementary vertical walls.
            if i==0:strip&=box(WALL_HANDOFF_X,-40,-.1,24-WALL_HANDOFF_X,80,12)
            elif i==8:strip&=box(terminal,-40,-.1,WALL_HANDOFF_X,80,12)
    ...
    lower_soft=fb&box(-.1,-40,-.1,24.1,80,12)
    upper_soft=Pos(-terminal,0,MAIN_SEAM)*(fb&box(terminal,-40,-.1,24.1,80,12))
    main_soft=lower_soft+upper_soft
    # Verify both butt joints have the entire 9.5 mm ribbon height on both sides.
    for sign in [-1,1]:
        gauge=box(WALL_HANDOFF_X-.01,sign*34.8-1.19,0,.02,2.38,DEPTH)
    ...
    old.package([('PET-GF15 shells and main cover',OUT/'print/rigid-strip.stl',False),
                ...
                OUT/'print/myo-flat-band.3mf','Myo clamshell band v0.10')
    ...
    report={'status':'PASS','units':'mm','revision':'0.10','wrist_reference_mm':wrist,
      ...
      'fasteners':{'main':'4 x M2 x 6 mm countersunk; 4 mm engagement, 1 mm bottom clearance',
      ...
      'assembly_seam':'Stepped clamshell: whole wrist floor and outer face, complementary full-height walls, 0.2 mm post clearance, 4 recessed screws',
      'cad_checks':['valid solids and expected STEP reimport counts','16 intended two-neighbor ribbon contacts',
                    ...
                    'full-height gauge across both ribbon butt joints',
      ...
      'limits':['No FEA, safe extension or spring-rate qualification','Ribbon butt ends meet at vertical wall handoff; physical closure and load transfer need trial',
                'No slicing, printing or fit evidence','Populated-board retention and harness strain relief remain unresolved']}
