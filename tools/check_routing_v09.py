...
for i in range(1,8):
    ...
    assert all(p.ToMM(t.GetWidth())>=.1524 for t in tracks)
    ...
    for t in tracks:
        a,z=t.GetStart(),t.GetEnd();dx,dy=abs(a.x-z.x),abs(a.y-z.y)
        assert min(dx,dy)<2000 or abs(dx-dy)<2000,('Non 45 segment',i)
    for v in vias:
        from plane_fanout import xy,point_rect
        for f in fps.values():
            for pad in f.Pads():
                bb=pad.GetBoundingBox();box=tuple(p.ToMM(x) for x in [bb.GetX(),bb.GetY(),bb.GetRight(),bb.GetBottom()])
                assert point_rect(xy(v.GetPosition()),box)>p.ToMM(v.GetDrillValue())/2,('Via in pad',i,f.GetReference(),pad.GetNumber())
    sat.append({'minimum_track_mm':min(p.ToMM(t.GetWidth()) for t in tracks),'via_in_pad':0,'non_45_segments':0,'pod':i,'segments':len(tracks),'vias':len(vias),'violations':0,'opens':0,'inner_signal_tracks':0})
old=load(HW/'backups/before-clean-pod-clone/haptic-bracelet.kicad_pcb');now=load(HW/'haptic-bracelet.kicad_pcb')
